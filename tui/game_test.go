...
func modelWithCheck(d levels.Difficulty, check engine.CheckRule) Model {
	return Model{
...
func TestSignalContractHiddenOnSurvival(t *testing.T) {
	m := modelWithCheck(levels.Survival, engine.CheckRule{StdoutEquals: "42"})
	if got := renderSignalContract(m); got != "" {
...
func TestSignalContractStdoutEquals(t *testing.T) {
	for _, d := range []levels.Difficulty{levels.Easy, levels.Medium, levels.Hard} {
		m := modelWithCheck(d, engine.CheckRule{StdoutEquals: "42"})
		got := renderSignalContract(m)
...
func TestSignalContractStdoutContains(t *testing.T) {
	m := modelWithCheck(levels.Easy, engine.CheckRule{StdoutContains: "ok"})
	got := renderSignalContract(m)
...
func TestSignalContractNonempty(t *testing.T) {
	m := modelWithCheck(levels.Easy, engine.CheckRule{StdoutNonempty: true})
	got := renderSignalContract(m)
...
func TestSignalContractEmptyCheck(t *testing.T) {
	m := modelWithCheck(levels.Easy, engine.CheckRule{})
	if got := renderSignalContract(m); got != "" {
